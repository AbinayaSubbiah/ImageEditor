...
import { EventHandler, Property, Event, compile, EmitType, KeyboardEvents, createElement, append } from '@syncfusion/ej2-base';
...
import { DropDownBase, dropDownBaseClasses, SelectEventArgs, FieldSettings, SPINNER_SIZE } from '../drop-down-base/drop-down-base';
...
export class DropDownList extends DropDownBase implements IInput {
    ...
    private isDocumentClick: boolean = false;
    protected isInteracted: boolean = false;
    private isFilterFocus: boolean = false;
    protected beforePopupOpen: boolean = false;
    protected initial: boolean = true;
    private initRemoteRender: boolean = false;
    ...
    private isNotSearchList: boolean = false;
    protected isTyped: boolean = false;
    protected isSelected: boolean = false;
    protected preventFocus: boolean = false;
    protected preventAutoFill: boolean = false;
    protected queryString: string;
    protected isValidKey: boolean = false;
    protected typedString: string = '';
    protected isEscapeKey: boolean = false;
    private isPreventBlur: boolean = false;
    protected isTabKey: boolean = false;
    private actionCompleteData: ActionCompleteData = { isUpdated: false };
    protected prevSelectPoints: { [key: string]: number } = {};
    protected isSelectCustom: boolean = false;
    protected isDropDownClick: boolean = false;
    protected preventAltUp: boolean = false;
        ...
        if (Object.keys(this.htmlAttributes).length) {
            for (let htmlAttr of Object.keys(this.htmlAttributes)) {
                if (htmlAttr === 'class') {
                    ...
                    let defaultAttr: string[] = ['title', 'id', 'name', 'placeholder', 'required'];
    ...
    protected getItemData(): { [key: string]: string } {
        ...
        let value: string = <string>(!isNullOrUndefined(dataItem) &&
            (dataValue === null || dataValue === '' || dataValue) ? dataValue : dataItem);
        let text: string = <string>(!isNullOrUndefined(dataItem) &&
            (dataText === null || dataText === '' || dataText) ? dataText : dataItem);
    ...
    protected setSearchBox(popupElement: HTMLElement): InputObject {
        if (this.isFiltering()) {
            let parentElement: HTMLElement = createElement('span', {
    ...
    private addNewItem(listData: { [key: string]: Object }[], newElement: HTMLElement): void {
        if (!isNullOrUndefined(this.itemData) && !isNullOrUndefined(newElement)) {
            ...
            let isExist: boolean = listData.some((data: { [key: string]: Object }) => {
                return getValue(this.fields.value, data) === value;
            });
            if (!isExist) {
            ...
            let ulElement: HTMLElement = this.actionCompleteData.ulElement;
            selectedItems = <NodeListOf<Element> & Element[]>ulElement.querySelectorAll('.' + dropDownBaseClasses.selected);
            if (selectedItems.length) {
                removeClass(selectedItems, dropDownBaseClasses.selected);
                selectedItems[0].removeAttribute('aria-selected');
            }
    ...
    protected renderPopup(): void {
        ...
        let popupEle: HTMLElement = createElement('div', { id: this.element.id + '_popup', className: 'e-ddl e-popup' });
    ...
    public render(): void {
        if (this.element.tagName === 'INPUT') {
            ...
            this.inputElement = createElement('input') as HTMLInputElement;
        ...
        for (let prop of Object.keys(newProp)) {
            switch (prop) {
                ...
                case 'width':
                    ...
                    break;
                case 'placeholder':
                    Input.setPlaceholder(newProp.placeholder, this.inputElement as HTMLInputElement);
                    break;
                ...
                case 'cssClass':
                    ...
                    if (this.popupObj) { this.popupObj.element.classList.add(newProp.cssClass); }
                    break;
                case 'enableRtl':
                    this.setEnableRtl();
                    break;
                case 'enabled':
                    ...
                    break;
                case 'text':
                    if (newProp.text === null) { this.clear(); return; }
                    if (!this.list) {
                    ...
                    break;
                case 'value':
                    if (newProp.value === null) { this.clear(); return; }
                    if (!this.list) {
                        if (this.dataSource instanceof DataManager) { this.initRemoteRender = true; }
                        this.renderList();
                    }
                    ...
                    break;
                case 'index':
                    if (newProp.index === null) { this.clear(); return; }
                    if (!this.list) {
                        if (this.dataSource instanceof DataManager) { this.initRemoteRender = true; }
                        this.renderList();
                    }
    ...
    public destroy(): void {
        ...
        ['readonly', 'aria-disabled', 'aria-placeholder', 'placeholder'].forEach((value: string): void => {
...
export interface FilteringEventArgs {
     ...
     * @return {void}.
     */
    updateData(dataSource: { [key: string]: Object }[] | DataManager | string[] | number[], query?: Query, fields?: FieldSettings): void;
}
